...
// Local fork ordering: ... account_stats_cost, session_id, kiro_credits, created_at
// (kiro_credits is a local Kiro patch column), so session_id is third-from-last.
func TestPrepareUsageLogInsert_SessionIDArgWiring(t *testing.T) {
	require.Len(t, usageLogInsertArgTypes, 60, "arg-type table must include response-model fields, session_id and kiro_credits")
	...
	// created_at is last, kiro_credits second-from-last; session_id precedes them.
	...
	require.Equal(t, "numeric", usageLogInsertArgTypes[len(usageLogInsertArgTypes)-2],
		"kiro_credits arg type must be numeric")
}
...
func TestPrepareUsageLogInsert_SessionIDNullWhenAbsent(t *testing.T) {
	...
	nsEmpty, ok := preparedEmpty.args[len(preparedEmpty.args)-3].(sql.NullString)
	require.True(t, ok, "session_id arg should be a sql.NullString")
