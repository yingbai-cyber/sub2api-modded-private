...
// Local fork ordering: ... session_id, native_compaction_v2, kiro_credits, created_at
func TestPrepareUsageLogInsert_SessionIDArgWiring(t *testing.T) {
	require.GreaterOrEqual(t, len(usageLogInsertArgTypes), 62, "arg-type table must include native_compaction_v2 and kiro_credits")
	...
	// created_at is last; kiro_credits penultimate; native_compaction_v2 before that; session_id precedes them.
...
func TestPrepareUsageLogInsert_SessionIDNullWhenAbsent(t *testing.T) {
	...
	nsEmpty, ok := preparedEmpty.args[len(preparedEmpty.args)-4].(sql.NullString)
