...
func (s *BillingService) computeTokenBreakdownForModel(
	...
	if normalizeBillingServiceTier(serviceTier) == "priority" && isOpenAIGPT55Model(model) {
		...
		if tierMultiplier == serviceTierCostMultiplier(serviceTier) {
...
func (s *BillingService) calculateCostInternalWithPolicy(
	...
	// 传 model 以支持 GPT-5.5 fast/priority 2.5x 修正；长上下文开关沿用调用方传入的 longContextBillingEnabled。
...
func (s *BillingService) applyModelSpecificPricingPolicyEx(model string, pricing *ModelPricing, forceDeepSeekRates bool) *ModelPricing {
	...
	if !needsCacheCreationPolicy && fastRatio <= 0 && !needsMaxReasoningEffortMultiplier && !usesLegacyLongContextPricing {
		return pricing
	}
	cloned := *pricing
