...
func TestAccountTestService_RunTestBackgroundOpenAIOAuthUsesWhamUsage(t *testing.T) {
	gin.SetMode(gin.TestMode)
	account := &Account{
		ID:          81,
		Platform:    PlatformOpenAI,
	...
	resp := newJSONResponse(http.StatusOK, `{
		...
		"rate_limit":{
			...
			"secondary_window":{"used_percent":34,"limit_window_seconds":604800,"reset_after_seconds":86400}
		},
	...
	result, err := svc.RunTestBackground(context.Background(), account.ID, "gpt-5.4-mini")
	require.NoError(t, err)
	require.Equal(t, "success", result.Status)
	require.Contains(t, result.ResponseText, "OpenAI usage probe succeeded")
	require.Len(t, upstream.requests, 1)
...
func TestAccountTestService_RunTestBackgroundOpenAIOAuthWhamForbiddenSetsError(t *testing.T) {
	gin.SetMode(gin.TestMode)
	account := &Account{
		ID:          82,
		Platform:    PlatformOpenAI,
	...
	resp := newJSONResponse(http.StatusForbidden, `{"detail":"account suspended"}`)
	repo := &openAIAccountTestRepo{
		mockAccountRepoForGemini: mockAccountRepoForGemini{
			accountsByID: map[int64]*Account{account.ID: account},
		},
	...
	require.Contains(t, result.ErrorMessage, "returned 403")
	require.Equal(t, account.ID, repo.setErrorID)
	require.Contains(t, repo.setErrorMsg, "Access forbidden (403)")
	require.Equal(t, http.StatusForbidden, repo.updatedExtra["openai_usage_probe_last_status"])
}

func TestAccountTestService_RunTestBackgroundOpenAIOAuthWhamLimitReachedSetsRateLimit(t *testing.T) {
	gin.SetMode(gin.TestMode)
	account := &Account{
		ID:          83,
		Platform:    PlatformOpenAI,
	...
	resp := newJSONResponse(http.StatusOK, `{
		...
		"rate_limit":{
			"allowed":false,
			"limit_reached":true,
			"primary_window":{"used_percent":100,"limit_window_seconds":18000,"reset_after_seconds":7200}
		},
		"credits":{"overage_limit_reached":false},
		"spend_control":{"reached":false},
	...
	require.NoError(t, err)
	require.Equal(t, "failed", result.Status)
	require.Contains(t, result.ErrorMessage, "usage limit reached")
	require.Equal(t, account.ID, repo.rateLimitedID)
	require.NotNil(t, repo.rateLimitedAt)
