...
func (s *GatewayService) ForwardAsResponses(
	...
	// 10-13: 发上游 + 读流的完整流程抽为闭包，外层做空流 retry。
...
retryLoop:
	for attempt := 1; attempt <= emptyStreamMaxAttempts; attempt++ {
		...
		if attempt < emptyStreamMaxAttempts {
			logger.L().Warn("forward_as_responses: upstream empty stream, retrying",
				zap.Int64("account_id", account.ID),
		...
		// 重试耗尽：统一由外层写 502 + 记 ops_error，和 buffered 错误路径对齐。
		logger.L().Warn("forward_as_responses: upstream empty stream after all retries",
			zap.Int64("account_id", account.ID),
			zap.String("model", mappedModel),
		...
		writeResponsesError(c, http.StatusBadGateway, "server_error", "Upstream returned empty stream")
...
//
// 关键不变量：HTTP 200 + SSE headers 的写入推迟到第一个有效事件处理之前。
...
func (s *GatewayService) handleResponsesStreamingResponse(
	...
	// 延迟到首个有效事件才写 headers。headersWritten 作为判定"是否已对
	// 客户端承诺 200"的单一事实来源。
	...
	processEvent := func(event *apicompat.AnthropicStreamEvent) bool {
		...
		if len(events) > 0 {
			// 首次确认要写业务事件 → 提交 200 + SE headers，之后 retry 窗口关闭。
	...
	// 空流检测：整个循环没触发过任何业务事件（headers 从未写出），
