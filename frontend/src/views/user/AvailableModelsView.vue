    ...
    <TablePageLayout>
      <template #actions>
        <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <div class="card p-5">
            <p class="text-sm font-medium text-gray-500 dark:text-dark-400">{{ t('availableModels.stats.groups') }}</p>
            <p class="mt-2 text-2xl font-semibold text-gray-900 dark:text-white">{{ summary.groupCount }}</p>
          </div>
...
import userModelsAPI, { type UserAvailableModelSection } from '@/api/models'
...
const sections = ref<UserAvailableModelSection[]>([])
...
const summary = computed(() => summarizeAvailableModels(sections.value))
const allRows = computed(() => flattenAvailableModelRows(sections.value))
