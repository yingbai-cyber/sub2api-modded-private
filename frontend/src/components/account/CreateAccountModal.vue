    ...
    >
      ...
      <div v-if="form.type === 'apikey' && form.platform !== 'antigravity'" class="space-y-4">
        ...
        <div>
          ...
          <!-- Probe Models Button - 紧跟 API Key 输入框 -->
          <div v-if="apiKeyValue.trim()" class="mt-2">
            <button
              type="button"
      ...
      <div v-if="form.platform === 'anthropic' && accountCategory === 'kiro'" class="space-y-4">
        ...
        <div>
          ...
          <div v-if="kiroBaseUrl.trim()" class="mt-2">
            <button
              type="button"
              :disabled="probingModels"
...
// 探测 Kiro 上游模型列表（通过 kiro-rs）
const handleKiroProbeModels = async () => {
  if (!kiroBaseUrl.value.trim()) return
  probingModels.value = true
  try {
    const result = await adminAPI.accounts.probeModels({
      platform: 'anthropic',
      base_url: kiroBaseUrl.value.trim(),
      api_key: kiroApiKey.value.trim() || '',
      proxy_id: form.proxy_id || undefined
    })
    if (result.models && result.models.length > 0) {
      const existing = new Set(allowedModels.value)
