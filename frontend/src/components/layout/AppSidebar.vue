  ...
  >
    ...
    <nav ref="sidebarNavRef" class="sidebar-nav scrollbar-hide">
      ...
      <template v-if="isAdmin">
        ...
        <div v-if="personalNavItems.length > 0" class="sidebar-section">
...
const flagAvailableModels = makeSidebarFlag(FeatureFlags.availableModels)
...
function buildSelfNavItems(withDashboard: boolean): NavItem[] {
  ...
  items.push(
    ...
    { path: '/available-models', label: t('nav.availableModels'), icon: ChannelIcon, featureFlag: flagAvailableModels },
    { path: '/monitor', label: t('nav.channelStatus'), icon: SignalIcon, featureFlag: flagChannelMonitor },
    { path: '/subscriptions', label: t('nav.mySubscriptions'), icon: CreditCardIcon, hideInSimpleMode: true },
    { path: '/purchase', label: t('nav.buySubscription'), icon: RechargeSubscriptionIcon, hideInSimpleMode: true, featureFlag: flagPayment },
    { path: '/orders', label: t('nav.myOrders'), icon: OrderListIcon, hideInSimpleMode: true, featureFlag: flagPayment },
    { path: '/announcements', label: t('nav.announcementCenter'), icon: BellIcon },
...
const adminNavItems = computed((): NavItem[] => {
  const baseItems: NavItem[] = [
    ...
    { path: '/admin/account-cleanup', label: t('nav.accountCleanup'), icon: GlobeIcon },
  ...
  // 简单模式下，在系统设置前插入常用个人入口和设置。
  if (authStore.isSimpleMode) {
    const filtered = visible.filter(item => !item.hideInSimpleMode)
    filtered.push({ path: '/keys', label: t('nav.apiKeys'), icon: KeyIcon })
    if (flagAvailableModels() !== false) {
      filtered.push({ path: '/available-models', label: t('nav.availableModels'), icon: ChannelIcon })
    }
