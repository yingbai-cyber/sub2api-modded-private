    ...
    <table class="w-full min-w-[760px] divide-y divide-gray-200 dark:divide-dark-700">
      ...
      <tbody v-if="loading" class="divide-y divide-gray-200 bg-white dark:divide-dark-700 dark:bg-dark-900">
        <tr v-for="i in 5" :key="i">
          <td v-for="column in 3" :key="column" class="px-5 py-4">
      ...
      <tbody v-else-if="rows.length === 0" class="bg-white dark:bg-dark-900">
        <tr>
          <td colspan="3" class="px-5 py-12 text-center text-gray-500 dark:text-dark-400">
        ...
        >
          <td class="px-5 py-4 align-top">
            <div class="flex flex-col gap-1">
              <span class="font-medium text-gray-900 dark:text-white">{{ row.model.display_name || row.model.name }}</span>
              <span v-if="row.model.display_name && row.model.display_name !== row.model.name" class="text-xs text-gray-500 dark:text-dark-400">
                {{ row.model.name }}
              </span>
            </div>
          </td>
...
import type { UserAvailableModelGroup } from '@/api/models'
...
function exclusiveGroups(row: AvailableModelRow): UserAvailableModelGroup[] {
  return row.groups.filter((group) => group.is_exclusive)
}

function publicGroups(row: AvailableModelRow): UserAvailableModelGroup[] {
  return row.groups.filter((group) => !group.is_exclusive)
}
</script>
